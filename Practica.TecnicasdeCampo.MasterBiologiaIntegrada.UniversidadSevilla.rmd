...
mydata <- read.csv("transectos.csv", header = T,  check.names = F)
mydata
mydata$ESPECIES <- corrected.species$Accepted_SPNAME
mydata$ESPECIES<- gsub(" ", "_", mydata$ESPECIES)
mydata
...
mytree$node.label <- NULL
mydata$Endemismo[mydata$ENDÉMICA == "Sí"] <- 1
mydata$Endemismo[mydata$ENDÉMICA == "No"] <- 0

myphylo.d <- phylo.d(mydata, mytree, ESPECIES, ENDÉMICA, permut = 1000, rnd.bias=NULL)
myphylo.d
